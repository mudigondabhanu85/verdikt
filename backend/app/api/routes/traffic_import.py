...
async def _resolve_relative_interaction_urls(
    session: AsyncSession, version_id: uuid.UUID, interactions: list
) -> None:
    """Some OpenAPI/Swagger documents — notably ones auto-generated by a
    framework like FastAPI that has no idea what URL it'll be deployed
    at — declare no `servers` (3.x) or `host` (2.0) at all, so
    OpenApiImporter's own base_url fallback chain (see
    app.importers.openapi_importer._base_url) bottoms out at "" and
    every interaction gets a bare relative path (e.g. "/reviews") as its
    URL. Left unresolved, ScopedHttpClient can never issue a request
    against these — found live: an OpenAPI import against exactly such
    a spec produced a scan that ran to completion "cleanly" with zero
    requests ever sent to any of the imported endpoints, a silent
    false-negative rather than a visible failure. Best-effort fix:
    resolve each bare path against this version's own first configured
    Target base_url — the same host every other agent already treats
    as "where this engagement's traffic goes" — leaving already-
    absolute URLs (every HAR/Postman/Burp capture has one) untouched.
    """
    if not any(not urlsplit(i.request.url).scheme for i in interactions):
        return
    result = await session.execute(
        select(Target).where(Target.version_id == version_id).order_by(Target.id).limit(1)
    )
    ...
    for interaction in interactions:
        if not urlsplit(interaction.request.url).scheme:
            interaction.request.url = f"{base}/{interaction.request.url.lstrip('/')}"
...
async def import_traffic(
    ...
    await _resolve_relative_interaction_urls(session, version_id, interactions)
