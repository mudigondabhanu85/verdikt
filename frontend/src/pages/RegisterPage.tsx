  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gray-50">
      <form onSubmit={handleSubmit} className="w-full max-w-sm rounded-lg border border-gray-200 bg-white p-8 shadow-sm">
        <h1 className="mb-2 text-xl font-semibold text-purple-700">Create your organization</h1>
        <p className="mb-6 text-sm text-gray-500">This account becomes the org_admin for a brand-new organization.</p>
        {error && <p className="mb-4 rounded bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>}
        <label className="mb-3 block text-sm">
          <span className="mb-1 block text-gray-600">Organization name</span>
          <input
            required
            value={orgName}
